...
class Transaction_reward_model extends CI_Model {
	function __construct() {
		parent::__construct();	
	}
		
	function select_all() {
		$this->db->select('t.*, e.emp_name, r.reward_name, p.position_name');
		$this->db->from('hrd_transaction_reward t');
		$this->db->join('hrd_employee e', 't.emp_id=e.emp_id');
		$this->db->join('hrd_reward r', 't.reward_id=r.reward_id');
	...
	function select_reward_employee($employee_id) {
	...
	function insert_data() {
		// Date
		$tanggal 	= trim($this->input->post('date_reward'));
		$xtanggal	= explode("-",$tanggal);
		$thn1 		= $xtanggal[2];
		$bln1 		= $xtanggal[1];
		$tgl1 		= $xtanggal[0];
		$date_reward= $thn1.'-'.$bln1.'-'.$tgl1;

		$data = array(    			
    			'emp_id' 			=> $this->input->post('lstEmployee'),
    			'trans_no' 			=> trim($this->input->post('reward_no')),
    			'reward_id' 		=> trim($this->input->post('lstReward')),
    			'trans_date' 		=> $date_reward,
		...
		$this->db->insert('hrd_transaction_reward', $data);
	}
	...
	function update_data() {
		...
		$this->db->update('hrd_transaction_reward', $data);
	}

	function delete_data($kode) {
		$this->db->where('trans_id', $kode);
		$this->db->delete('hrd_transaction_reward');		
	}		
}
/* Location: ./application/model/rew-pun/Transaction_reward_model.php */